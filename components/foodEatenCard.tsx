...
export default function FoodEatenCard ({ meals, mealGroups } :
    ...
    return (
        <WhiteCard className="px-6">
            <div className="mb-2.5 font-medium text-sm flex justify-between">
                <div>
            ...
            <div className="w-full bg-gray-lighter rounded-full h-2 mb-1.5">
                ...
                style={{width: `${Math.min(percentage, 100)}%`}}>
                    <div className={`absolute inset-y-0 m-auto -right-2.5 
                    w-5 h-5 rounded-full bg-white border-2 flex border-primary`}>
                        <i className="fa-solid fa-paw text-primary-dark m-auto"/>
                    </div>

                    <div className="absolute inset-y-0 w-5 m-auto -right-2.5 text-center text-gray mt-4">
                        {todayMealsFoodEaten}
                    </div>
                </div>
            </div>
